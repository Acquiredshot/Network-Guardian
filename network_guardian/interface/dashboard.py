...
class Dashboard:
    ...
    def _page_reports(self) -> str:
        ...
        html = (
            ...
            '--red:#f85149;--orange:#db6d28;--yellow:#d29922;--green:#3fb950;--blue:#58a6ff;--purple:#bc8cff}'
            '*{box-sizing:border-box;margin:0;padding:0}'
            'body{background:var(--bg);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;font-size:.9rem}'
            '.wrap{max-width:1400px;margin:0 auto;padding:20px}'
            '.banner{display:flex;align-items:center;gap:12px;padding:16px 20px;background:var(--surface);'
            'border:1px solid var(--border);border-radius:10px;margin-bottom:16px}'
            '.banner h1{font-size:1.3rem;font-weight:700}'
            ...
            '.nav a{padding:6px 14px;border-radius:20px;text-decoration:none;color:var(--dim);'
            'background:var(--surface);border:1px solid var(--border);font-size:.82rem;transition:.15s}'
            '.nav a:hover,.nav a.active{background:#1f6feb22;color:var(--blue);border-color:var(--blue)}'
            '.card{background:var(--surface);border:1px solid var(--border);border-radius:10px;'
            'padding:18px;margin-bottom:16px}'
            '.card h2{font-size:1rem;font-weight:600;margin-bottom:14px;color:var(--blue)}'
            '.report-card{background:var(--bg);border:1px solid var(--border);border-radius:8px;'
            ...
            '.report-header:hover{background:#ffffff08}'
            '.report-body{padding:16px;display:none}'
            '.report-body.open{display:block}'
            '.pill{display:inline-block;padding:2px 8px;border-radius:10px;font-size:.7rem;font-weight:600}'
            '.sev-critical{background:#f8514920;color:var(--red)}'
            '.sev-high{background:#db6d2820;color:var(--orange)}'
            '.sev-medium{background:#d2992220;color:var(--yellow)}'
            '.sev-low{background:#3fb95020;color:var(--green)}'
            '.sev-info{background:#58a6ff20;color:var(--blue)}'
            ...
            'background:#ffffff05;border-radius:0 6px 6px 0}'
            ...
            '.label{font-size:.7rem;font-weight:600;color:var(--dim);text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px}'
            '.value{font-size:.85rem;line-height:1.5}'
            '.action-block{border-left:3px solid var(--blue);padding:10px 14px;margin:8px 0;'
            'background:#58a6ff08;border-radius:0 6px 6px 0}'
            '.narrative{background:#1f6feb15;border:1px solid #1f6feb40;border-radius:6px;'
            ...
            '.obs-item{background:#ffffff06;border-radius:6px;padding:8px 10px;text-align:center}'
            '.obs-item .n{font-size:1.3rem;font-weight:700}'
            '.obs-item .l{font-size:.65rem;color:var(--dim);margin-top:2px}'
            '.empty{color:var(--dim);padding:30px;text-align:center}'
            '.filter-row{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px}'
            '.filter-btn{padding:5px 12px;border-radius:16px;border:1px solid var(--border);'
            'background:var(--surface);color:var(--dim);cursor:pointer;font-size:.78rem}'
            '.filter-btn.active{border-color:var(--blue);color:var(--blue);background:#58a6ff15}'
